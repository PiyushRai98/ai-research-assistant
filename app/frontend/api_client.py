...
class APIClient:
    ...
    def document_file_bytes(self, document_id: str) -> bytes:
        return self._request("GET", f"/documents/{document_id}/file").content
...
class ResearchClient(Protocol):
    """Structural interface shared by :class:`APIClient` and the in-process
    ``EmbeddedClient``, so Streamlit views can accept either without caring
    which transport (HTTP or direct in-process calls) is active.
    """
    ...
    def literature_review(
        self, *, document_ids: list[str], topic: str | None
    ) -> dict[str, Any]: ...
    def export_chat(self, session_id: str, *, fmt: str) -> bytes: ...
